import React, { useEffect, useRef } from "react";

// Компонент обёртка для amoCRM формы
// Форма и вся логика остаются на стороне amoCRM, мы только подгружаем их скрипт
//
// ВНИМАНИЕ (внешняя зависимость): после успешной отправки формы amoCRM
// делает redirect на страницу /thanks (маршрут задан в App.jsx). Сам redirect
// настраивается НЕ в этом коде, а в настройках формы №1672210 в кабинете amoCRM
// (Форма → Настройки → «Действие после отправки» → URL перенаправления).
// Если меняете путь /thanks — обязательно обновите его и в настройках amoCRM.

function AmoForm() {
  const containerRef = useRef(null);

  useEffect(() => {
    // Если скрипт уже инициализирован, второй раз не подключаем
    if (document.getElementById("amoforms_script_1672210")) {
      return;
    }

    // Вставляем инлайновый конфигурационный скрипт (как в index.html)
    const inlineScript = document.createElement("script");
    inlineScript.type = "text/javascript";
    inlineScript.innerHTML = `!(function (a, m, o, c, r, m2) {
      (a[o + c] = a[o + c] || {
        setMeta: function (p) {
          this.params = (this.params || []).concat([p]);
        },
      }),
        (a[o + r] =
          a[o + r] ||
          function (f) {
            a[o + r].f = (a[o + r].f || []).concat([f]);
          }),
        a[o + r]({
          id: "1672210",
          hash: "0757972e108e37d8f2bdaed4117c5743",
          locale: "ru",
        }),
        (a[o + m2] =
          a[o + m2] ||
          function (f, k) {
            a[o + m2].f = (a[o + m2].f || []).concat([[f, k]]);
          });
    })(window, 0, "amo_forms_", "params", "load", "loaded");`;

    // Подключаем внешний скрипт amoCRM
    const externalScript = document.createElement("script");
    externalScript.id = "amoforms_script_1672210";
    externalScript.async = true;
    externalScript.charset = "utf-8";
    externalScript.src =
      "https://forms.amocrm.ru/forms/assets/js/amoforms.js?1771497082";

    const current = containerRef.current || document.body;
    current.appendChild(inlineScript);
    current.appendChild(externalScript);

    // Ничего не чистим на unmount, чтобы форма не пересоздавалась при мелких рендерах
  }, []);

  return <div ref={containerRef} />;
}

export default AmoForm;
